...
float4 main(VSOutput input) : SV_TARGET
{
    ...
    float4 texColor0 = GaussianBlur(tex0, uv);
	//texColor0.r = tex0.Sample(smp, uv + float2(rgbUvNum, 0.f)).r;
    texColor0.r = GaussianBlur(tex0, uv + float2(rgbUvNum, 0.f)).r;
    float4 texColor1 = GaussianBlur(tex1, uv);
	//texColor1.r = tex1.Sample(smp, uv + float2(rgbUvNum, 0.f)).r;
    texColor1.r = GaussianBlur(tex1, uv + float2(rgbUvNum, 0.f)).r;
